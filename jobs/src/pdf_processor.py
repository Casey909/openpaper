...
async def process_pdf_file(
    ...
    try:
        ...
        source_name = original_filename or s3_object_key
        source_ext = Path(source_name).suffix.lower()
        temp_suffix = source_ext if source_ext else ".pdf"
        is_pdf = temp_suffix == ".pdf"
        ...
        safe_filename = f"file-{job_id}{temp_suffix}"

        # Extract text and page offsets from PDF
        try:
            async with time_it("Extracting text and offsets from document", job_id=job_id):
                pdf_text = await extract_text(
                    temp_file_path,
                )
                status_callback(f"Processed bits and bytes")
                logger.info(f"Extracted {len(pdf_text)} characters of text from document")
                page_offsets = (
                    map_pages_to_text_offsets(temp_file_path) if is_pdf else {}
                )
        ...
        logger.info(f"Document already uploaded to S3: {file_url}")
        ...
        logger.info(f"Document processing completed successfully for {safe_filename} in {duration:.2f} seconds")
